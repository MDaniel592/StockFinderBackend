...
from utils.unalix._config import httpopt, data_min, body_redirects
from utils.unalix._exceptions import (
    ConnectError,
    InvalidURL,
    InvalidScheme,
    TooManyRedirects,
)
from utils.unalix._http import (
    add_missing_attributes,
    create_connection,
    create_cookie_jar,
    handle_redirects,
)
from utils.unalix._utils import (
    is_private,
...
def parse_url(url):
    ...
    scheme, netloc, path, params, query, fragment = urlparse(prepend_scheme_if_needed(url, "http"))
...
def unshort_url(url, parse_documents=False, cookie_policy="allow_if_needed", **kwargs):
    ...
    while True:
        ...
        if total_redirects > httpopt.max_redirects:
            raise TooManyRedirects(message="The request exceeded maximum allowed redirects", url=url)
    ...
    for file in files:
        ...
        for provider in ruleset:
            ...
            for rule in ruleset[provider]["rules"]:
                rules.append(re.compile(pattern=rf"(%(?:26|23)|&|^){rule}(?:(?:=|%3[Dd])[^&]*)"))

            for referral in ruleset[provider]["referralMarketing"]:
                referrals.append(re.compile(pattern=rf"(%(?:26|23)|&|^){referral}(?:(?:=|%3[Dd])[^&]*)"))
    ...
    for file in files:
        ...
        for rule in ruleset:
            ...
            for redirect in redirects:
                new_redirects.append(re.compile(pattern=f".*{redirect}.*", flags=re.MULTILINE | re.DOTALL))
